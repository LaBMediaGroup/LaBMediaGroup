...
var BANDS=[
 {id:'band-make',  title:'Learn &amp; <em>Make</em>',
  blurb:'Reference material and the tools you point at a problem \u2014 craft, AI, sound, type, motion and code.',
  groups:['references','ai','music','stock','fonts','3d','software']},
 {id:'band-who',   title:'Find People &amp; <em>Community</em>',
 ...
 {id:'band-gear',  title:'Find Gear &amp; <em>Flight</em>',
...
function detailHTML(r){
  ...
  if(r.additionalLinks && r.additionalLinks.length){
    out+='<div class="d-links"><span class="d-lbl">Also worth knowing</span>'
      + r.additionalLinks.map(function(l){
  ...
  if(r.features && r.features.length>3){
    out+='<div class="d-feat">'+r.features.map(function(f){
...
function itemHTML(r,i){
  ...
  var more = d ? '<details><summary>More'+hint+'</summary><div class="d-panel">'+d+'</div></details>' : '';
...
/* A saved kit needs a stable key per entry. Only 21 of the 128 entries carry
   an explicit id, but all 128 names are unique and slugify uniquely — so the
...
/* ============================================================
   ...
   So every page searches all 128 and anything that belongs to a
...
(function render(){
  ...
  /* `claimed` is exactly what this page rendered, so "new" is counted from that
     ...
     lives on one of the other two. And the count says "n of 128" so the slice
...
/* ============================================================
   ...
   no longer renders the list, which used to sit above the 128
...
(function(){
  ...
  /* ---- Matches on the other two pages ----
     ...
     page. Now it isn't, so a query is run against all 128 and anything that
