...
Notes: (1) Trail Dead (#horror): Added background image 'images/Traildead.jpg' with Ken Burns zoom animation (26s duration). Combined subtle noise texture with background image using layered background-image. Added radial gradient mood overlay (dark vignette) with red accent line using combined ::after pseudo-element. Content sits at z-index 3 above overlays. (2) Lookout (#comedy): Added background image 'images/Lookout.jpg' with Ken Burns zoom animation (22s duration). Added warm gradient overlay (yellows to browns) with transparency to let image show through. Content sits at z-index 3. (3) Added reduced-motion safety: Both #horror::before and #comedy::before animations disabled with 'animation: none !important' in prefers-reduced-motion media query. Red accent line maintained for horror section (no glow, crisp line only).
