...
2026-01-16 | 6:03PM EST
———————————————————————
Change: Darkened the Anthony Brass overlay, collapsed the DFA announcement by default with a nudge, and corrected the callboard nav logo.
Files touched: portfolio.html, events.html, callboard.html, CHANGELOG_RUNNING.md
Notes: Reduced overlay opacity with a warm tint, added a subtle 20s attention animation, and fixed the nav logo asset.
Quick test checklist:
1. Open portfolio.html and confirm the Anthony Brass overlay is darker but still transparent.
2. Open events.html and confirm the announcement starts collapsed, the toggle nudges subtly, and expands/collapses correctly.
3. Open callboard.html and verify the nav logo uses the white beaker.
