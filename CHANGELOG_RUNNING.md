...
2026-01-10 | (Current Session) - Final Pre-Upload Fixes
———————————————————————
Change: Fixed events not loading on index page and added comprehensive debugging for favorites/events features.
Files touched: index.html, resources.html
Notes:
- Fixed: Events preview not loading on index.html - changed `eventsData` to `window.eventsData` (line 1735)
- Added: Comprehensive console logging for debugging favorites save button
- Added: Console logging for events loading process
- Debug logs help identify any issues with toast notifications, button clicks, or event loading
- All debugging prefixed with [SAVE], [TOAST], or [EVENTS] for easy filtering
Quick test checklist:
- Open index.html → Events preview section shows next 3 upcoming events
