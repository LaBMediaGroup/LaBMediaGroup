(function(){
  ...
  var UI_KEY='lab-flight-checklist-ui-v1';
  var groups=[
  ...
  var mode=savedUI.mode||'minimized';
...
#lab-flight-checklist[data-mode="closed"]{width:auto;filter:none}\
...
.fc-actions{display:flex;gap:4px}.fc-icon,.fc-launch,.fc-text-btn{border:1px solid var(--line-2,rgba(255,255,255,.2));background:var(--paper-2,#181818);color:var(--dim,#bbb);cursor:pointer}.fc-icon{width:32px;height:32px;font:13px var(--f-mono,monospace)}.fc-icon:hover,.fc-icon:focus-visible,.fc-launch:hover,.fc-launch:focus-visible,.fc-text-btn:hover,.fc-text-btn:focus-visible{border-color:var(--accent,#e66f3c);color:var(--ink,#fff);outline:none}\
.fc-progress{height:2px;background:var(--line,rgba(255,255,255,.1))}.fc-progress span{display:block;height:100%;background:var(--accent,#e66f3c);transition:width .25s ease}.fc-body{max-height:min(62vh,570px);overflow:auto;padding:12px 15px 16px}.fc-group+.fc-group{margin-top:14px}.fc-group h3{margin:0 0 5px;padding-bottom:6px;border-bottom:1px solid var(--line,rgba(255,255,255,.1));font:8px var(--f-mono,monospace);letter-spacing:.15em;text-transform:uppercase;color:var(--faint,#aaa)}\
.fc-item{display:grid;grid-template-columns:15px 1fr;gap:9px;align-items:start;padding:6px 0;color:var(--dim,#bbb);font-size:11px;line-height:1.45;cursor:pointer}.fc-item input{position:absolute;opacity:0}.fc-box{width:13px;height:13px;margin-top:1px;border:1px solid var(--line-2,rgba(255,255,255,.2));position:relative}.fc-item input:focus-visible+.fc-box{outline:2px solid var(--accent,#e66f3c);outline-offset:2px}.fc-item input:checked+.fc-box{background:var(--accent,#e66f3c);border-color:var(--accent,#e66f3c)}.fc-item input:checked+.fc-box:after{content:"";position:absolute;left:3px;top:0;width:4px;height:8px;border:solid var(--paper,#111);border-width:0 1.5px 1.5px 0;transform:rotate(45deg)}.fc-item input:checked~span:last-child{color:var(--faint,#888);text-decoration:line-through}.fc-foot{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:13px;padding-top:11px;border-top:1px solid var(--line,rgba(255,255,255,.1))}.fc-text-btn{padding:7px 9px;font:8px var(--f-mono,monospace);letter-spacing:.1em;text-transform:uppercase}.fc-privacy{font:8px/1.4 var(--f-mono,monospace);color:var(--faint,#888)}\
#lab-flight-checklist[data-mode="minimized"]{width:min(270px,calc(100vw - 24px))}#lab-flight-checklist[data-mode="minimized"] .fc-body,#lab-flight-checklist[data-mode="minimized"] .fc-progress{display:none}#lab-flight-checklist[data-mode="minimized"] .fc-head{min-height:46px;border-bottom:0;padding-block:6px}#lab-flight-checklist[data-mode="minimized"] .fc-title{font-size:17px}#lab-flight-checklist[data-mode="minimized"] [data-fc="print"]{display:none}#lab-flight-checklist[data-mode="closed"] .fc-panel{display:none}.fc-launch{display:none;padding:10px 13px;font:9px var(--f-mono,monospace);letter-spacing:.12em;text-transform:uppercase;box-shadow:0 8px 22px rgba(0,0,0,.2)}#lab-flight-checklist[data-mode="closed"] .fc-launch{display:block}.fc-launch b{color:var(--accent,#e66f3c);font-weight:400}\
...
@media(prefers-reduced-motion:reduce){.fc-progress span{transition:none}}\
@media print{body.lab-print-checklist>*:not(#lab-flight-checklist){display:none!important}body.lab-print-checklist #lab-flight-checklist{position:static!important;width:100%!important;filter:none!important;color:#111!important}body.lab-print-checklist .fc-panel{display:block!important;border:1px solid #777!important;background:#fff!important;backdrop-filter:none!important}body.lab-print-checklist .fc-head{cursor:default!important}body.lab-print-checklist .fc-actions,body.lab-print-checklist .fc-foot,body.lab-print-checklist .fc-progress{display:none!important}body.lab-print-checklist .fc-body{display:block!important;max-height:none!important;overflow:visible!important}body.lab-print-checklist .fc-title,body.lab-print-checklist .fc-item{color:#111!important}body.lab-print-checklist .fc-group h3,body.lab-print-checklist .fc-sub{color:#555!important}body.lab-print-checklist .fc-item input:checked+.fc-box{background:#111!important;border-color:#111!important}body.lab-print-checklist .fc-item input:checked+.fc-box:after{border-color:#fff!important}}';
  ...
  var groupHTML=groups.map(function(group){
    return '<section class="fc-group"><h3>'+esc(group.title)+'</h3>'+group.items.map(function(item){
  ...
  root.innerHTML='<div class="fc-panel"><div class="fc-head" data-fc-drag><div><h2 class="fc-title">Flight <em>checklist.</em></h2><span class="fc-sub"><b data-fc-count>0</b> of 18 ready · saved here</span></div><div class="fc-actions"><button class="fc-icon" type="button" data-fc="print" title="Save or print PDF" aria-label="Save or print checklist as PDF">PDF</button><button class="fc-icon" type="button" data-fc="minimize" title="Minimize" aria-label="Minimize checklist">—</button><button class="fc-icon" type="button" data-fc="close" title="Close" aria-label="Close checklist">×</button></div></div><div class="fc-progress"><span data-fc-fill></span></div><div class="fc-body">'+groupHTML+'<div class="fc-foot"><span class="fc-privacy">Browser-only · not a regulatory document</span><button class="fc-text-btn" type="button" data-fc="reset">Reset</button></div></div></div><button class="fc-launch" type="button" data-fc="open">Flight checklist · <b data-fc-launch-count>0/18</b></button>';
  ...
  function sync(){
    ...
    root.querySelector('[data-fc-launch-count]').textContent=checked.length+'/'+boxes.length;
    root.querySelector('[data-fc-fill]').style.width=(checked.length/boxes.length*100)+'%';
  }
  function setMode(next){root.dataset.mode=next;persistUI()}
